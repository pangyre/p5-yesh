...
There are quite a few Perl packages that must be installed. If everything is working right you should be able to download the distribution for Yesh, decompress it, and install it like any Perl package. If you have not installed Catalyst and Moose before, the installation list will be quite long.
...
POST request with username and email? Sends a token URI to the user to return to the same page where-
...
L<Yesh::Manual::ReleaseNotes>, L<Yesh::Manual::SampleSetups>.
